...
from diffusers import StableDiffusionXLControlNetPipeline, ControlNetModel, UNet2DConditionModel, LCMScheduler
...
pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
    ...
    unet = UNet2DConditionModel.from_pretrained("latent-consistency/lcm-sdxl", torch_dtype=preferred_dtype),
...
if preferred_device == "cpu" and not keep_fp32_on_cpu:
    ...
    # qint8(pipe.text_encoder_2, inplace=True)
...
for iteration in tqdm(iteration_range):
    if demo_mode:
        synthetic_time = datetime.fromtimestamp(iteration * 15 * 60 // 4 + 8 * 3600 - 2 * 15 * 60 // 4)
